  ...
  .action((option) => {
    ...
    tagPromptProcess.on('message', (message) => {
      ...
      if (!_v) {
        ...
        childProcess.spawnSync('git', ["fetch", "origin", "--prune"])
        const stdout = childProcess.spawnSync('git', ['tag', '-l', `${branch}*`])
      ...
      childProcess.spawnSync('git', ['tag', '-a', currentTag, '-m', description])
...
`
      ...
      // childProcess.spawnSync('git', ['push', 'origin', `${currentTagArr.join('-')}`])
  ...
  .action((option) => {
    if (!projects || !projects.length) {
      ...
      runPromptProcess.on('message', (message) => {
        ...
        // 打开vscode
        childProcess.spawnSync('goo', ['code', identify], {stdio: 'inherit'})
...
if (process.argv.length === 2) {
  childProcess.spawnSync('goo', ['run'], {stdio: 'inherit'}, (err, stdout, stdio) => {
    if (err) {
